...
// ================================================================================
// Utils https://github.com/jshafto/poemlapse
// ================================================================================
...
class App extends React.Component {
  ...
  constructor(props) {
    ...
    this.toggleFocusModeTooltipException =
      this.toggleFocusModeTooltipException.bind(this);
  ...
  componentDidMount() {
    ...
    if (typeof ipc !== "undefined") {
      ...
      ipc.on("menu.cursorFollowingOff", () =>
        this.setState({ cursorFollowing: false })
      );
      ipc.on("menu.cursorFollowingOn", () =>
        this.setState({ cursorFollowing: true })
      );
      ...
      ipc.on("openFileFromFileIcon", (evt, path) =>
        this.openFileFromFileIcon(path)
      );
    }
  ...
  modelDraftMarker({ description, name, timelineIndex, timestamp }) {
    const defaultName =
      "Draft " + (this.state.editorHighestEverDraftNumber + 1);
  ...
  toggleFocusModeTooltipException() {
    const appFocusModeTooltipException =
      !this.state.appFocusModeTooltipException;
    this.setState({ appFocusModeTooltipException });

    if (
      appFocusModeTooltipException &&
      !$("body").hasClass("focus-mode-tooltip-exception")
    ) {
  ...
  play() {
    if (
      this.state.editorTimelineIndex >=
      this.state.editorTimelineFrames.length - 1
    ) {
      this.setPos(0);
    }
  ...
  async checkForUnsavedChanges(message) {
    if (this.state.editorHasUnsavedChanges) {
      const res = await remote.getGlobal("confirm")(
        message ||
          "The current document contains unsaved changes. Proceed anyway?",
        ["Ok", "Cancel"]
      );
  ...
  async saveFileAs() {
    const fileInfo = await remote.getGlobal("saveFileAs")(
      this.modelMidstFile()
    );
  ...
  load(fileData) {
    this.setState(
      {
        appTitle: fileData.fileName
          ? fileData.fileName.replace(this.FILE_EXT, "")
          : "Untitled",
        ...
        editorHighestEverDraftNumber:
          fileData.data.meta.editorHighestEverDraftNumber || 0,
        editorHasUnsavedChanges: false,
        appFileAbsPath: fileData.path,
        editorFontFamily: fileData.data.meta.editorFontFamily || "Helvetica",
        editorFontSize:
          fileData.data.meta.editorFontSize || this.defaultFontSize,
      },
      () => {
        this.$editable.html(
          _.get(_.last(this.state.editorTimelineFrames), "content")
        );
        this.$editable.focus();
    ...
    this.setState(
      {
        ...
        editorDraftMarkers: editorDraftMarkers.concat([
          this.modelDraftMarker({ timelineIndex: editorTimelineIndex }),
        ]),
      },
      () => {
  ...
  editDraftMarkerLabel(timelineIndex, inDrawer) {
    return () => {
      const id =
        "draft-marker-" + timelineIndex + (inDrawer ? "-in-drawer" : "");
      const marker = _.find(this.state.editorDraftMarkers, { timelineIndex });

      this.setState({
        editorEditingDraftMarker: timelineIndex + (inDrawer ? "-drawer" : ""),
      });
  ...
  markerIndexFromTimelineIndex(timelineIndex) {
    const { editorDraftMarkers } = this.state;
    return editorDraftMarkers.findIndex(
      ({ timelineIndex: index }) => index === timelineIndex
    );
  }
  ...
  scrollCursorIntoView() {
    if (this.state.appCursorFollowing) {
      const currentFrame =
        this.state.editorTimelineFrames[this.state.editorTimelineIndex];

      if (currentFrame) {
        const currentLineNumber = parseInt(currentFrame.lineNumber);
        const $currentLine = this.$editable.find(
          '[data-line-number="' + currentLineNumber + '"]'
        );
        const $fifthLineAbove = this.$editable.find(
          '[data-line-number="' + (currentLineNumber - 5) + '"]'
        );
    ...
    return e(
      ...
      e(
        ...
        e(
          ...
          {
            className:
              "round-icon focus-mode-toggle tooltip" +
              (appFocusMode ? " active" : ""),
            title: "Focus mode",
  ...
  renderEditor() {
    ...
    return e(
      ...
      e("div", {
        id: "editable",
        className:
          (appDrawerOpen && !appFocusMode ? "with-drawer" : "") +
          (appTimelineMode ? " with-timeline" : ""),
        contentEditable:
          !editorCreatingDraftMarker && !editorEditingDraftMarker && !isPlayer,
      })
    );
  }
  ...
  renderBottomToolbar() {
    return e(
      ...
      e(
        "div",
        { className: "double-icon timeline-toggles" },
        e(
          "div",
          {
            className:
              "round-icon timeline-toggle" +
              (this.showTimeline() ? "" : " deactivated"),
  ...
  renderDraftMarkerCreateIcon() {
    ...
    const markerIndices = editorDraftMarkers.map(
      (marker) => marker.timelineIndex
    );
    ...
    return e(
      ...
      {
        className:
          "round-icon draft-marker-create" +
          ((markerExists && !editorCreatingDraftMarker) || !this.showTimeline()
            ? " deactivated"
            : "") +
          (editorCreatingDraftMarker ? " active" : "") +
          (appDrawerOpen && editorDraftMarkers.length < 1
            ? " shake animated"
            : ""),
        onMouseDown:
          !markerExists && !editorCreatingDraftMarker
            ? this.createDraftMarker
            : null,
      },
      iconMarker()
    );
  ...
  renderDraftMarkers() {
    const {
      ...
      editorTimelineFrames,
      editorEditingDraftMarker,
      editorTimelineIndex: currentTimelinePosition,
      editorShowDraftMarkerLabels,
    } = this.state;
      ...
      editorDraftMarkers.map(({ timelineIndex, name }, markerNo) => {
        ...
        return e(
          "div",
          {
            ...
            className:
              "draft-marker" +
              (editorEditingDraftMarker === timelineIndex.toString()
                ? " editing"
                : "") +
              (active ? " active" : ""),
            style: {
          ...
          editorShowDraftMarkerLabels
            ? this.renderDraftMarkerLabel(
                name || "Draft " + (markerNo + 1),
                timelineIndex,
                false,
                active
              )
  ...
  renderDraftMarkerLabel(name, timelineIndex, inDrawer, active) {
    return e(
      "div",
      {
        className:
          "draft-marker-label" +
          (this.state.editorEditingDraftMarker ===
          timelineIndex + (inDrawer ? "-drawer" : "")
            ? " editing"
            : ""),
      ...
      e(
        "span",
        {
          onClick:
            active && !inDrawer
              ? this.editDraftMarkerLabel(timelineIndex, inDrawer)
              : null,
        },
        name
      ),
  ...
  renderTimelineControlsPlayer() {
    ...
    return e(
      ...
      e(
        ...
        {
          className:
            "round-icon timeline-button-3" + (editorPlaying ? " active" : ""),
      ...
      e(
        ...
        {
          className:
            "round-icon timeline-button-2 speed-selector" +
            (appDrawerOpen ? " active" : ""),
        ...
        e(
          Drop,
          {
            ...
            onDropToggled: () => {
              this.setState({
                playerPlaybackSpeedDropOpen: !playerPlaybackSpeedDropOpen,
              });
            },
          },
          e(
  ...
  renderTimelineControlsApp() {
    ...
    return e(
      ...
      e(
        ...
        {
          className:
            "round-icon timeline-button-2" + (appDrawerOpen ? " active" : ""),
          onClick: this.toggleDrawer,
        },
        iconDrawer()
      ),
  ...
  renderDrawer() {
    ...
    return e(
      ...
      reversedMarkers.length < 1
        ? e(
            "div",
            { className: "empty-drawer-message" },
            e(
              "p",
              {},
              e("span", {}, "Your Drawer"),
      ...
      e(
        "div",
        { className: "marker-list-controls" },
        e(
          "div",
          {
            ...
            onClick: () =>
              this.setState({
                editorShowDraftMarkers: !editorShowDraftMarkers,
              }),
          },
        ...
        e(
          "div",
          {
            ...
            onClick: () =>
              this.setState({
                editorShowDraftMarkerLabels: !editorShowDraftMarkerLabels,
              }),
          },
